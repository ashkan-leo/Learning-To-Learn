...
def restore_trained_optimizer_variables():
    ...
    with g1.as_default():
        with tf.Session() as sess:
            ...
            # I need to extract all model's parameters here and then construct
            # another graph
...
def build_optimizer_graph():
    ...
    for i in range(num_of_coordinates):
        cell_list.append(tf.nn.rnn_cell.MultiRNNCell(
            [lstm_cell] * num_layers))  # num_layers = 2 according to the paper.
    batch_size = 1
    state_list = [cell_list[i].zero_state(batch_size, tf.float32)
                  for i in range(num_of_coordinates)]
    sum_f = 0
    g_new_list = []
    for i in range(num_of_coordinates):
        ...
        for k in range(n_unroll_in_m):
            ...
            cell_output, state = cell(
                grad_h_t, state)  # g_new should be a scalar b/c grad_h_t is a scalar
        ...
        # state_list[i] = state # for the next t # I don't need this list
        # right..? b/c I'm not using t...T thing.

    # Reshaping g_new
    g_new = tf.reshape(tf.squeeze(tf.pack(g_new_list)), [
                       n_dimension, 1])  # should be a [10, 1] tensor
...
def build_training_graph(method):
    ...
    for i in range(n):
        W_i = tf.reshape(
            tf.slice(W, begin=[i, 0, 0], size=[1, n_dimension, n_dimension]), [n_dimension, n_dimension])
        y_i = tf.reshape(
            tf.slice(y, begin=[i, 0, 0], size=[1, n_dimension, 1]), [n_dimension, 1])
        f = tf.reduce_sum(tf.square(tf.matmul(W_i, theta) - y_i))
                          # make this faster using tensor only
        loss += f
    loss /= (n * n_dimension)
    ...
    if method == "lstm":
        ...
        train_op = tf.assign(theta, new_value)
                             # just to make it compatiable with method == "SGD
                             # case.
    ...
    with g.as_default():
        with tf.Session() as sess:
            if args.optim_method == "lstm":
                loss_op, f_grad_op, train_op, g_new_ph, W_ph, y_ph = build_training_graph(
                    method=args.optim_method)
                g_op, f_grad_ph = build_optimizer_graph()
            elif args.optim_method == "SGD":
                loss_op,  train_op, W_ph, y_ph = build_training_graph(
                    method=args.optim_method)
            else:
            ...
            for var in tf.trainable_variables():
                ...
                if var.name in variable_dict:
                    assign_op = var.assign(
                        variable_dict[var.name])  # the inside param has to be a np array like this: var.assign(np.ones(12))
                    sess.run(assign_op)
            ...
            if args.optim_method == "lstm":
                ...
                for epoch in range(max_epoch):
                    loss_val, f_grad_val, _ = sess.run(
                        [loss_op, f_grad_op, train_op], feed_dict={g_new_ph: g_new_val, W_ph: W_val, y_ph: y_val})
                    g_new_val = sess.run(
                        g_op, feed_dict={f_grad_ph: f_grad_val})
                    print(loss_val)
                    cost_list.append(loss_val)
            if args.optim_method == "SGD":
                for epoch in range(max_epoch):
                    loss_val, _ = sess.run(
                        [loss_op, train_op], feed_dict={W_ph: W_val, y_ph: y_val})
                    print(loss_val)
                    cost_list.append(loss_val)
